...
- (IBAction)resetUsedTasks:(id)sender {
    
    // prompt for password
    
    // get contents of Tasks folder
    NSString *taskPath = [@"~/Library/Logs/Discipline/Tasks" stringByExpandingTildeInPath];
    NSArray *taskFolderContents = [[NSFileManager defaultManager] directoryContentsAtPath:taskPath];
    
    // delete anything that has "Used" in the range of its name
    NSMutableArray *tasksToDelete = [NSMutableArray array];
    for (NSString *taskName in taskFolderContents) {
        if ([taskName rangeOfString:@"Used"].length > 0) {
            [tasksToDelete addObject:[taskPath stringByAppendingPathComponent:taskName]];
        }
    }
    for (NSString *taskToDelete in tasksToDelete) {
        [[NSFileManager defaultManager] removeItemAtPath:taskToDelete error:nil];
    }
}
