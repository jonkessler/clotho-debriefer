...
- (IBAction)nextButtonClicked:(id)sender {
    ...
    [dataToLog logData:[[[questionData taskList] objectAtIndex:0] objectForKey:@"breakpoint"] 
                forKey:@"breakpoint"];
...
- (NSArray *)createDebrieferDataArray:(NSString *)withDate {
    ...
    withDate = [[NSDate dateWithNaturalLanguageString:withDate] description];
    NSString *testDate = @"";
    int i;
    for (i=0; i<[taskFileList count]; i++) {
        NSDateFormatter *formatter = [[[NSDateFormatter alloc] init] autorelease];
        [formatter setDateFormat:@"yyyy-MM-dd"];
        testDate = [formatter stringFromDate:[NSDate dateWithNaturalLanguageString:withDate]];        
        while ([[taskFileList objectAtIndex:i] rangeOfString:testDate].length == 0) {
