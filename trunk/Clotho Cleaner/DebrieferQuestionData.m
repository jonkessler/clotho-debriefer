...
- (NSArray *)retrieveApps {
    ...
    for (i=0; i<[formattedTimes count]; i++) {
        ...
        for (NSDictionary *aDict in systemSnapshot) {
            ...
            if ([appToBeAdded isEqualToString:@"Clotho Logger"])
                continue;
...
- (void)divideTasksAndApps:(NSMutableArray *)combinedAndShuffled {
    ...
    //  Since we are going to be debriefed on these questions, put them in the UsedTasks file
    NSString *usedTasksPath = 
    [[[@"~/Library/Logs/Discipline/Tasks/UsedTasks_" stringByAppendingString:dataDate]
      stringByAppendingPathExtension:@"plist"] stringByExpandingTildeInPath];
    NSMutableArray *justTheTasks = [NSMutableArray arrayWithArray:[self arrayFromPlist:usedTasksPath]];

    for (NSDictionary *task in taskList) {
        NSString *taskAndDate = [[task objectForKey:@"task"] stringByAppendingString:
                                 [[task objectForKey:@"date"] description]];
        [justTheTasks addObject:[taskAndDate stringByReplacingOccurrencesOfString:@" " withString:@""]];
    }
    [justTheTasks writeToFile:usedTasksPath atomically:NO];
...
- (NSArray *)retrieveTasksForFile:(NSString *)fileName {
    ...
    NSMutableArray *usedTasks = [NSMutableArray arrayWithArray:[self arrayFromPlist:usedTasksPath]];
    NSArray *tasks = 
    ...
    //  put tasks into tasksAndDates, excluding all "--idle--"s and previously used
    for (id item in tasks) {
        NSString *theTask = [item objectForKey:@"name"];
        NSDate *theDate = [item objectForKey:@"date"];
        NSString *theBreakPoint = [item objectForKey:@"breakpoint"];
        shouldBeUsed = [self taskShouldBeUsed:theTask forDate:theDate fromData:usedTasks];

        if (shouldBeUsed) {
            NSDictionary *taskToAdd = [NSDictionary dictionaryWithObjectsAndKeys:
                                       theTask, @"task", 
                                       [theDate description], @"date", 
                                       theBreakPoint, @"breakpoint", nil];
...
- (void)shuffleTasks {
    ...
    if (taskCount == 0) {
        [self setTaskList:[NSMutableArray array]];
        [self setAppsList:[NSMutableArray array]];
        return;
    }
    
    while (taskCount > 1) {
        NSInteger randomNumber = (random() % taskCount);     //  picks a random number from 0 to taskCount
...
- (BOOL)taskShouldBeUsed:(NSString *)theTask forDate:(NSDate *)theDate fromData:(NSArray *)theUsedTasks {
    ...
    BOOL ifShouldBeUsed;
    NSString *taskAndDate = [[theTask stringByAppendingString:[theDate description]] 
                             stringByReplacingOccurrencesOfString:@" " withString:@""];
    
    //  check theTask against these conditions
    if ([theTask isEqualToString:@"--idle--"] || [theTask isEqualToString:@""]) {
        ifShouldBeUsed = NO;
    }
    else if ([theUsedTasks containsObject:taskAndDate]) {
        ifShouldBeUsed = NO;
    }
    else
        ifShouldBeUsed = YES;
    
    return ifShouldBeUsed;
}
